...
export const cxAnimation = (startValue: number, endValue: number, during = 1000, callback: (v: number) => any, type: keyof typeof animeType = 'easeInBack') => {
  ...
  function step() {
    const stepValue = startValue + changeValue * animeType.easeOutSine(position);
    callback(stepValue);
