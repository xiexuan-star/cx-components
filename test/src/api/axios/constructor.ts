...
import { AxiosInterceptor, CxAxiosOptions } from './type';
import { isFunction } from '@/utils/is';
import { CANCEL_REQUEST } from './const';
import Qs from 'qs';

export class CxAxios {
  ...
  private setInterceptor() {
    ...
    this.instance?.interceptors.request.use(config => {
      ...
      if (invalid) {
        console.warn(
          `CxAxios: can't send same request (${config.url} `,
          config.params,
          `) before a request responded`
        );
  ...
  private request<T = any>(config: AxiosRequestConfig, options: AnyObject) {
    ...
    return new Promise((resolve, reject) => {
      this.instance?.request(config).then(
        async res => {
          res = isFunction(beforeResponseHook) ? await beforeResponseHook(res, options) : res;
          resolve(((res?.data ?? {}) as unknown) as SResponse<T>);
        },
        err => {
          isFunction(beforeResponseErrHook) && beforeResponseErrHook(err, options);
          reject(err);
        }
      );
